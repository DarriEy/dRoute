...
MassBalanceResult test_mass_balance(Router& router, int num_timesteps, double pulse_magnitude) {
    ...
    for (int t = 0; t < num_timesteps; ++t) {
        ...
        // Accumulate outflow
        for (int out : outlets) {
            result.total_outflow += router.get_discharge(out) * dt;
        }
    }
    
    // Estimate final storage (sum of current flows * reach length / celerity)
    // This is approximate but should be small after pulse has passed
    result.final_storage = 0.0;
    for (int id : net.topological_order()) {
        double Q = router.get_discharge(id);
        result.final_storage += Q * dt;  // Simplified storage estimate
    }
