...
/**
 * Test mass balance: ∑Q_out × Δt + ΔS = ∑Q_in × Δt
 * 
 * Pulse inflow, no lateral inflow, simple chain network.
 */
bool test_mass_balance() {
    ...
    for (int t = 0; t < num_timesteps; ++t) {
        ...
        net.get_reach(1).lateral_inflow = Real(inflow);
        ...
        total_outflow += to_double(net.get_reach(3).outflow_curr) * config.dt;
    }
    
    // Compute storage change (sum of current flows in network × some characteristic time)
    // For a simple test, we just run long enough that storage returns to ~0
    double storage_final = 0.0;
    for (int id : net.topological_order()) {
        storage_final += to_double(net.get_reach(id).outflow_curr) * config.dt * 10;  // Rough estimate
    }
    ...
    // Allow 5% imbalance (some water still in transit)
...
bool test_lateral_inflow_gradient() {
    ...
    // Run simulation with lateral inflow in reach 2
    auto run_sim = [&]() {
        for (int id : net.topological_order()) {
            Reach& r = net.get_reach(id);
            r.inflow_prev = r.inflow_curr = Real(0.0);
        ...
        for (int t = 0; t < 50; ++t) {
            // Headwater inflow
            net.get_reach(1).lateral_inflow = Real(50.0);
            // Lateral inflow to middle reach
            net.get_reach(2).lateral_inflow = Real(30.0);
